...
- **2026-08-19/20, second batch — moves 29, 31, 32, 33, 28, 38, 39, 40, 41, 10, 7, 21, 24 DONE and verified live.** Deck: reconnects on foreground and greets once (`test/deck-link.test.js`); phone layout (dvh/safe-area/16px/bottom-tabs/tap-expand); iOS voice (`u.lang` always set, voices reloaded on gesture, ⚙ VOICE sheet); honest LIVE·BASIC MODE badge; installable PWA + `sw.js` off-tailnet page. PC: fifteen read-only verbs + `pc.snapshot` + `screen.capture` (verified live box→PC→deck, `cpu.top` 3–10 s, screenshot 1.5 s), `shell.read` (provably read-only, refuses mutations up front), `shell` shows its whole command + spoken summary + inbox log, late-result watcher, long-poll claim (~1 s round-trip). Memory: `notifications` index + hourly retention; durable push caps across processes; registry-driven platform URLs/names (davenroe/gluecron now visible, metrics probes all 9); `brain_turns` telemetry table + summary endpoint; `session-end.sh`/`/memory/session/end` made honest. 544 tests, lint clean. Commits `78bfac7`…`(move 24)`. **Remaining unattended candidates:** 14 (clock + memory pen), 15 (one brain session), 17 (model routing), 18 (FTS memory), 19 (vision), 20 (MCP connectors), 34 (web push), 36 (deck refactor), 44–47 (org efficiency).

- **2026-08-19 (earlier), commits `00b2188` + `f4d6894` — moves 2, 3, 4, 6, 43, 13, 16 DONE and verified live.** `authHold()`/`spawnHold()` consumed by orchestrator (re-queue + park), code-health, harvester, review-runner; `hasClaudeBrain()` is liveness; brain heartbeat writes `claude-last-spawn-ok`; basic mode is safe (the "love you what" utterance now gets an instant honest "basic mode" reply and stages nothing — measured 0.0 s on the live deck); total-outage alert once per UTC day; review-runner decides each artifact once (KV + info inbox row) and a hold stops the tick without moving the cursor; situation synthesis has a 10-min floor; native WebSearch ON for the brain (verified `used_search=yes` on the subscription), WebFetch deliberately still off; effort `medium` on Opus 5 / `high` on Fable 5. 512 tests green on the box, lint clean.

- **2026-08-20, security + brain-capability batch — moves 37, 11, 14, 17, 15 DONE and verified live.** **37** server-side PC gate: mutating `/pc/action` needs an HMAC confirmation token (secret stripped from agents) — verified no-token 403, forged 403, legit runs, replay 409, read-only unaffected. **11** internal-token auth on the loopback control plane: `/dispatch`, finding writes and proposal-transition are 403 without the token, benign writes/reads open, no legit service refused. **14** the memory pen: `remember`/`recall`/`set_reminder`/`list_reminders` + an NZ clock + a 30 s reminder poller — verified end to end (a reminder set 40 s out fired, notified, marked once). **17** model routing: Haiku for verify/distill/situation/recheck/verdict, Sonnet for the finder, Opus for repairs (was Fable), Fable escalation — all four IDs validated on 158's login. **15** cross-surface continuity so a warm session sees the other surface's interleaved turns. 568 tests green, all 11 services up. Two new secrets (`JARVIS_PC_CONFIRM_SECRET`, `JARVIS_INTERNAL_TOKEN`) generated on the box. **Remaining candidates:** 5, 8, 9, 12, 18, 19, 20, 25, 26, 30, 34–36, 42, 44–50.

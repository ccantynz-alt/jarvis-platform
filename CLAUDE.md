...
| jarvis-deck | src/deck-server.js | 9210 | loopback, exposed ONLY via `tailscale serve --https=8444` | **Command Deck HUD** (2026-07-16, from Craig's Claude Design handoff) — 4-view telemetry deck (HUD/Hierarchy/Message Flow/Platforms) at public/command-deck.html + raw WS `/jarvis` speaking the handoff's contract v1.0 (+ `chat_chunk` streaming, `notify` spoken alerts, `org` extension). All numbers real (9200/9205/9209 + platform-health.json + own probes). Commands → lib/agent.js brain (**claude-fable-5**) with intent fallback; conversation persisted in memory KV `deck-conversation`. Voice: wake word "Jarvis" (fuzzy), `GET /tts` = ElevenLabs neural voice via src/lib/tts.js (cache + daily budget + `TTS_DISABLED` kill switch), browser speechSynthesis fallback. Token = JARVIS_DECK_TOKEN or JARVIS_GATEWAY_TOKEN (gateway cookie also accepted). |

...
const ZOOBICON = process.env.ZOOBICON_URL || 'https://zoobicon.com';
...
const executors = {
  ...
  async build(state) {
    ...
    let html = null, sectionCount = 0;
    // SSE frames: lines of "data: {json}"; the {"type":"done"} event carries html.
    const text = await r.text();
    ...
    if (!html || html.length < 500) return { ok: false, error: `builder stream ended with no usable html (${html ? html.length : 0} bytes)` };
    const files = {
      'index.html': html,
      'serve.ts': [
    ...
    return { ok: true, result: { files, sectionCount, mock: args.mock, bytes: html.length, mocked: false } };
  },
  ...
  async push(state) {
    ...
    const files = state.stages.build.result.files;
    ...
    try {
      ...
      git(...authed.slice(0), 'push', '--quiet', 'origin', 'HEAD:main');
